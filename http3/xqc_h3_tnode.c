...
int xqc_tnode_hash_table_free_node(xqc_tnode_hash_table_t * table){
    ...
    for(i = 0; i < table->element_count; i++){
        ...
        xqc_list_for_each_safe(pos, next, head){
            struct xqc_http3_tnode_t * tnode = xqc_list_entry(pos, xqc_http3_node_t, head_list);
            xqc_list_del(pos);
            free(tnode);
        }
    }
...
#if 0
    head_list->prev->next = head_list->next;
    head_list->next->prev = head_list->prev;
...
int xqc_tnode_insert_to_hash(xqc_tnode_hash_table * table, xqc_http3_tnode_t * tnode){
...
int xqc_http3_node_id_init(xqc_http3_node_id_t * nid, xqc_http3_node_id_type type, uint64_t id){
    nid->type = type;
    nid->id = id;
    return 0;
...
xqc_http3_tnode_t * xqc_http3_create_tnode( xqc_tnode_hash_table_t * table, xqc_http3_node_id * nid, uint64_t seq,  uint32_t weight, xqc_http3_tnode_t * parent){

    xqc_http3_tnode_t * tnode = malloc(sizeof(xqc_http3_tnode_t));
    if(tnode == NULL){
        return NULL;
    }
    xqc_http3_tnode_init(tnode, nid, seq, weight, parent);
...
void xqc_http3_tnode_free_root(xqc_http3_tnode_t * root){
}


int xqc_http3_tnode_init(xqc_http3_tnode_t * tnode, xqc_http3_node_id * nid, uint64_t seq,  uint32_t weight, xqc_http3_tnode_t * parent){
