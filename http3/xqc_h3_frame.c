    ...
    switch(stream_type){
        case XQC_HTTP3_STREAM_TYPE_CONTROL:
            if (h3_conn->flags & XQC_HTTP3_CONN_FLAG_CONTROL_OPENED) {
                ...
                XQC_CONN_ERR(h3_conn->conn, HTTP_STREAM_CREATION_ERROR);
            ...
            case XQC_HTTP3_CTRL_STREAM_STATE_FRAME_LENGTH:
                ...
                if(!(h3_conn->flags & XQC_HTTP3_CONN_FLAG_SETTINGS_RECVED)){
                    ...
                    if(rstate->fr.hd.type != XQC_HTTP3_FRAME_SETTINGS) {
                        XQC_CONN_ERR(h3_conn->conn, HTTP_MISSING_SETTINGS);
                    ...
                    XQC_CONN_ERR(h3_conn->conn, HTTP_SETTINGS_ERROR);
...
int xqc_http3_stream_send_buf_add(xqc_h3_stream_t * h3_stream, xqc_h3_frame_send_buf_t * send_buf){
...
int xqc_http3_uni_stream_write_stream_type(xqc_h3_stream_t * h3_stream, uint8_t stream_type){
    ...
    int ret = xqc_http3_stream_send_buf_add(h3_stream, send_buf);
    if(ret < 0){
        return ret;
    }
    return 0;
...
int xqc_http3_send_frame_buffer(xqc_h3_stream_t * h3_stream, xqc_list_head_t * head){ //return 1 means send buf completely, return 0 means buf data send imcompletely
    ...
    xqc_list_for_each_safe(pos, next, head){
        ...
        ssize_t send_success = xqc_stream_send(h3_stream->stream, send_buf->data + send_buf->already_consume, send_buf->data_len - send_buf->already_consume, (send_buf->fin_flag & XQC_HTTP3_STREAM_FIN));
...
int xqc_http3_frame_data_buffer_and_send(xqc_h3_stream_t * h3_stream, xqc_h3_frame_send_buf_t * send_buf){//control_data_send do not return eagain, if eagian, buf data, if error ,return error
...
ssize_t xqc_http3_write_frame_header(xqc_h3_stream_t * h3_stream, char * data, ssize_t data_len, uint8_t fin){
    ...
    ret = xqc_http3_frame_data_buffer_and_send(h3_stream, send_buf);
    if(ret < 0){
        return ret;
    }
    return data_len;
...
ssize_t xqc_http3_write_frame_data(xqc_h3_stream_t * h3_stream, char * data, ssize_t data_len, uint8_t fin){
    ...
    if(fin_only){
        ...
        if(send_success == -XQC_EAGAIN){
            ...
            ret = xqc_http3_stream_send_buf_add(h3_stream, send_buf);
            if(ret < 0){
                return ret;
            }
            return  0;
    ...
    while(data_len > 0){
        ...
        ssize_t send_success = xqc_stream_send(h3_stream->stream, send_buf->data, send_buf->data_len, send_buf->fin_flag & XQC_HTTP3_STREAM_FIN );
...
int xqc_http3_stream_write_settings(xqc_h3_stream_t * h3_stream, xqc_http3_conn_settings * settings ){
    ...
    int ret = xqc_http3_frame_data_buffer_and_send(h3_stream, send_buf);
    return ret;
}
