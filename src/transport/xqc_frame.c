...
xqc_process_new_conn_id_frame(xqc_connection_t *conn, xqc_packet_in_t *packet_in)
{
    ...
    if (xqc_cid_in_dcid_set(&new_conn_cid, &conn->dcid_set)) {
        return XQC_OK;
    }

    xqc_dcid_set_insert_cid(&conn->dcid_set, &new_conn_cid, XQC_CID_UNUSED);
...
xqc_process_new_token_frame(xqc_connection_t *conn, xqc_packet_in_t *packet_in)
{
    ...
    conn->engine->eng_callback.save_token(conn->conn_token, conn->conn_token_len, xqc_conn_get_user_data(conn));
    ...
    xqc_packet_in_t *packet_in)
{
    ...
    xqc_int_t ret = xqc_parse_path_status_frame(packet_in, &path_id, 
                                                &path_status, &path_status_seq, &path_prio);
    if (ret != XQC_OK) {
        xqc_log(conn->log, XQC_LOG_ERROR,
    ...
    if (path == NULL) {
        ...
        xqc_cid_t *dcid = xqc_get_dcid_by_seq(&conn->dcid_set, path_id);
        if (dcid == NULL) {
            xqc_log(conn->log, XQC_LOG_ERROR,
