...
{
    return ((alpn_len == lengthof(XQC_ALPN_H3) && memcmp(alpn, XQC_ALPN_H3, lengthof(XQC_ALPN_H3)) == 0)
        || (alpn_len == lengthof(XQC_ALPN_H3_29) && memcmp(alpn, XQC_ALPN_H3_29, lengthof(XQC_ALPN_H3_29)) == 0));
}

xqc_bool_t
xqc_alpn_type_is_transport(const unsigned char *alpn, uint8_t alpn_len)
{
    return (alpn_len == lengthof(XQC_ALPN_TRANSPORT)
        && memcmp(alpn, XQC_ALPN_TRANSPORT, lengthof(XQC_ALPN_TRANSPORT)) == 0);
}
...
{
    return ((alpn_len == lengthof(XQC_ALPN_HQ_INTEROP) && memcmp(alpn, XQC_ALPN_HQ_INTEROP, lengthof(XQC_ALPN_HQ_INTEROP)) == 0)
        || (alpn_len == lengthof(XQC_ALPN_HQ_29) && memcmp(alpn, XQC_ALPN_HQ_29, lengthof(XQC_ALPN_HQ_29)) == 0));
}
