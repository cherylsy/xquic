...
#define BBR2_INFINITE_RTT 0x7fffffff
#define xqc_bbr2_kCycleLength 8
/*Size of window of bandwidth filter, in rtts */
static const uint32_t xqc_bbr2_kBandwidthWindowSize = xqc_bbr2_kCycleLength + 2;
/*Window of min rtt filter, in sec */
static const uint32_t xqc_bbr2_kMinRttWindowSize_msec = 2500000; //2.5s
static const uint32_t xqc_bbr2_probe_rtt_min_win_msec = 2500000; //2.5s
/* Minimum time spent in bbr2_PROBE_RTT, in usec*/
static const uint32_t xqc_bbr2_kProbeRttTime = 200000; //200ms
/*Initial rtt before any samples are received, in usec  */
static const uint64_t xqc_bbr2_kInitialRtt = 100;
/*The gain of pacing rate for STRAT_UP, 2/(ln2) */
static const float xqc_bbr2_kHighGain = 2.885;
/*Gain in bbr2_DRAIN */
//static const float xqc_bbr2_kDrainGain = 1.0 / 2.885;
static const float xqc_bbr2_kDrainGain = 0.75;
/* Gain for cwnd in probe_bw, like slow start*/
static const float xqc_bbr2_kCwndGain = 2.0;
/*Cycle of gains in PROBE_BW for pacing rate */
static const float xqc_bbr2_kPacingGain[] = {1.25, 0.75, 1, 1, 1, 1, 1, 1};
/*Minimum packets that need to ensure ack if there is delayed ack */
static const uint32_t xqc_bbr2_kMinCongestionWindow = 4 * XQC_kMaxDatagramSize;
/*If bandwidth has increased by 1.25, there may be more bandwidth avaliable */
static const float xqc_bbr2_kFullBandWithThresh = 1.25;
/*After 3 rounds bandwidth less than (1.25x), estimate the pipe is full */
static const uint32_t xqc_bbr2_kFullBandwidthCnt = 3;
...
//BBRv2 parameters
static const float xqc_bbr2_kStartupCwndGain = 2;
static const bool xqc_bbr2_extra_ack_in_startup = 1;
static const uint32_t xqc_bbr2_extra_ack_win_rtt = 5; //10 packet-timed rtt
static const uint32_t xqc_bbr2_extra_ack_win_rtt_in_startup = 1; //2 packet-timed rtt
static const float xqc_bbr2_kStartupPacingGainOnLost = 1.5; //slow down
static const float xqc_bbr2_inflight_lo_beta = 0.3;
static const uint8_t xqc_bbr2_full_loss_cnt = 8;
static const float xqc_bbr2_loss_thresh = 0.02; //2%
...
static void xqc_bbr2_enter_probe_refill(xqc_bbr2_t *bbr2, xqc_sample_t *sampler, uint32_t cnt);
static bool xqc_bbr2_is_inflight_too_high(xqc_bbr2_t *bbr2, xqc_sample_t *sampler);
static void xqc_bbr2_set_cycle_idx(xqc_bbr2_t *bbr2, xqc_bbr2_pacing_gain_phase idx);
...
size_t xqc_bbr2_size()
{
    return sizeof(xqc_bbr2_t);
}

static void xqc_bbr2_enter_startup(xqc_bbr2_t *bbr2)
{
    bbr2->mode = BBR2_STARTUP;
    bbr2->pacing_gain = xqc_bbr2_kHighGain;
    bbr2->cwnd_gain = xqc_bbr2_kStartupCwndGain;
}

static void xqc_bbr2_init_pacing_rate(xqc_bbr2_t *bbr2,xqc_sample_t *sampler)
{
    uint64_t bandwidth;
    if(sampler->srtt)
        bbr2->has_srtt = 1;
    bandwidth = bbr2->congestion_window * (uint64_t)msec2sec / (sampler->srtt ? sampler->srtt : 1000);
    bbr2->pacing_rate = bbr2->pacing_gain * bandwidth;
}

static void xqc_bbr2_reset_congestion_signals(void *cong_ctl)
{
...
static void xqc_bbr2_init(void *cong_ctl, xqc_sample_t *sampler, xqc_cc_params_t cc_params)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*)(cong_ctl);
    uint64_t now = xqc_now();
    memset(bbr2, 0, sizeof(*bbr2));
    bbr2->min_rtt = sampler->srtt ? sampler->srtt : BBR2_INFINITE_RTT;
    bbr2->min_rtt_stamp = now;
    bbr2->probe_rtt_min_us = sampler->srtt ? sampler->srtt : BBR2_INFINITE_RTT;
    ...
    bbr2->prior_cwnd = 0;
    bbr2->initial_congestion_window = 32 * XQC_kMaxDatagramSize; // same as cubic
...
static uint32_t xqc_bbr2_max_bw(xqc_bbr2_t *bbr2)
{
    return xqc_max(bbr2->bw_hi[0], bbr2->bw_hi[1]);
}

static void xqc_bbr2_update_round_start(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
...
static void xqc_bbr2_calculate_bw_sample(xqc_sample_t *sampler, xqc_bbr2_context_t *ctx)
{
    if (sampler->delivered < 0 || sampler->interval <= 0) return;
    /*Calculate the new bandwidth, bytes per second */
    ctx->sample_bw = 1.0 * sampler->delivered / sampler->interval * msec2sec;
}

static uint32_t xqc_bbr2_bdp(xqc_bbr2_t *bbr2, uint32_t bw) {
    if (bbr2->min_rtt == BBR2_INFINITE_RTT)
        return bbr2->initial_congestion_window;
    uint64_t w = bbr2->min_rtt * (uint64_t)bw;
    return (uint32_t)(w / msec2sec);
}

static uint32_t xqc_bbr2_inflight(xqc_bbr2_t *bbr2, uint32_t bw, float gain)
{
    uint32_t bdp = xqc_bbr2_bdp(bbr2, bw);
    return (uint32_t)(bdp * gain); 
}

static void xqc_bbr2_advance_bw_hi_filter(xqc_bbr2_t *bbr2) {
    if (!bbr2->bw_hi[1]) return;
    bbr2->bw_hi[0] = bbr2->bw_hi[1];
    bbr2->bw_hi[1] = 0;
}

static void xqc_bbr2_probe_inflight_hi_upward(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    uint32_t delta = 0;
    ...
    if (sampler->prior_inflight < bbr2->congestion_window) {
        not_cwnd_limited = (bbr2->congestion_window - sampler->prior_inflight) >= XQC_kMaxDatagramSize;
    }
    //not cwnd_limited or ...
	...
	bbr2->bw_probe_up_acks += sampler->acked / XQC_kMaxDatagramSize;
	if (bbr2->bw_probe_up_acks >=  bbr2->bw_probe_up_cnt) {
		...
		bbr2->inflight_hi += delta * XQC_kMaxDatagramSize;
	}

    xqc_log(sampler->send_ctl->ctl_conn->log, XQC_LOG_DEBUG, "|increase_inflight_hi|probe_up_acks:%ud|newlyacked:%ud"
        "|probe_up_cnt:%ud|inflight_hi:%ud|delta:%ud|probe_up_rounds:%ud|",
...
static void xqc_bbr2_handle_inflight_too_high(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
...
static bool xqc_bbr2_adapt_upper_bounds(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
	...
	if (xqc_bbr2_is_inflight_too_high(bbr2, sampler)) {
		...
		if (bbr2->inflight_hi == ~0U)  /* no excess queue signals yet? */
			return false;
...
static uint32_t xqc_bbr2_bw(xqc_bbr2_t *bbr2)
{
    return xqc_min(xqc_bbr2_max_bw(bbr2), bbr2->bw_lo);
}

uint32_t xqc_bbr2_target_inflight(xqc_bbr2_t *bbr2)
{
	uint32_t bdp = xqc_bbr2_bdp(bbr2, xqc_bbr2_bw(bbr2));
	return xqc_min(bdp, bbr2->congestion_window);
}

static bool xqc_bbr2_is_reno_coexistence_probe_time(xqc_bbr2_t *bbr2)
{
    ...
    if (reno_gain) {
        inflight_in_pkts = xqc_bbr2_target_inflight(bbr2) / XQC_kMaxDatagramSize;
...
static void xqc_bbr2_reset_lower_bounds(xqc_bbr2_t *bbr2)
{
    bbr2->bw_lo = ~0U;
    bbr2->inflight_lo = ~0U;
}

void xqc_bbr2_enter_probe_refill(xqc_bbr2_t *bbr2, xqc_sample_t *sampler, uint32_t cnt)
{
    xqc_bbr2_reset_lower_bounds(bbr2);
    if (bbr2->inflight_hi != ~0U)
    ...
    bbr2->cwnd_gain = xqc_bbr2_kCwndGain;
    bbr2->pacing_gain = xqc_bbr2_kPacingGain[bbr2->cycle_idx];
}

static bool xqc_bbr2_has_elapsed_in_phase(xqc_bbr2_t *bbr2, uint64_t now, uint64_t usec)
{
    return (now > (usec + bbr2->cycle_start_stamp));
}

static bool xqc_bbr2_check_time_to_probe_bw(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    uint64_t now = sampler->now;
    if (xqc_bbr2_has_elapsed_in_phase(bbr2, now, bbr2->probe_wait_us) || xqc_bbr2_is_reno_coexistence_probe_time(bbr2)) {
        xqc_bbr2_enter_probe_refill(bbr2, sampler, 0);
        return true;
    }
    return false;
...
void xqc_bbr2_raise_inflight_hi_slope(xqc_bbr2_t *bbr2)
{
	...
	cnt = (bbr2->congestion_window / XQC_kMaxDatagramSize) / growth_this_round;
	cnt = xqc_max(cnt, 1U);
	bbr2->bw_probe_up_cnt = cnt;
}

static void xqc_bbr2_enter_probe_up(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
...
static uint32_t xqc_bbr2_inflight_with_headroom(xqc_bbr2_t *bbr2)
{
    if (bbr2->inflight_hi == ~0U)
        return ~0U;
	...
	headroom = xqc_max(headroom, XQC_kMaxDatagramSize);
    if (bbr2->inflight_hi >= headroom) //to avoid uint underflow
        inflight_wo_headroom = bbr2->inflight_hi - headroom;
	return xqc_max(inflight_wo_headroom, xqc_bbr2_kMinCongestionWindow);
}

static bool xqc_bbr2_check_time_to_cruise(xqc_bbr2_t *bbr2, uint32_t inflight, uint32_t bw)
{
    bool is_under_bdp;
...
static void xqc_bbr2_enter_probe_cruise(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    if (bbr2->inflight_lo != ~0U)
...
static void xqc_bbr2_update_cycle_phase(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
	...
	case BBR2_BW_PROBE_UP:
        ...
        xqc_log(sampler->send_ctl->ctl_conn->log, XQC_LOG_DEBUG, "|PROBE_UP_GO|inflight:%ud|target:%ud|queuing:%ud|bw:%ud|", 
            inflight, xqc_bbr2_inflight(bbr2, bw, xqc_bbr2_bw_probe_up_gain), bw);
	...
	default:
		xqc_log(bbr2->send_ctl->ctl_conn->log, XQC_LOG_WARN, "|BBR invalid cycle index %ud|", bbr2->cycle_idx);
	}
}


static uint32_t xqc_bbr2_extra_ack(xqc_bbr2_t *bbr2)
{
    return xqc_max(bbr2->extra_ack[0], bbr2->extra_ack[1]);
}


static uint32_t xqc_bbr2_ack_aggregation_cwnd(xqc_bbr2_t *bbr2)
{
    uint32_t max_aggr_cwnd, aggr_cwnd = 0;
    if(xqc_bbr2_extra_ack_gain && (bbr2->full_bandwidth_reached || bbr2->extra_ack_in_startup))
    {
...
static void xqc_update_ack_aggregation(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    ...
    if (bbr2->epoch_ack <= expected_ack
        || (bbr2->epoch_ack + sampler->acked >= xqc_bbr2_ack_epoch_acked_reset_thresh)) {
        bbr2->epoch_ack = 0;
        bbr2->extra_ack_stamp = sampler->now;
        expected_ack = 0;
    }
    /* Compute excess data delivered, beyond what was expected. */
    uint32_t cap = 0xFFFFFU * XQC_kMaxDatagramSize;
...
static void xqc_bbr2_check_full_bw_reached(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    ...
    if(!bbr2->round_start || bbr2->full_bandwidth_reached || sampler->is_app_limited ){
        return;
    }

    uint32_t bw_thresh = bbr2->last_bandwidth * xqc_bbr2_kFullBandWithThresh;
    ...
    bbr2->full_bandwidth_reached = bbr2->full_bandwidth_cnt >= xqc_bbr2_kFullBandwidthCnt;
}

static void xqc_bbr2_enter_drain(xqc_bbr2_t *bbr2)
{
    bbr2->mode = BBR2_DRAIN;
    //bbr2->pacing_gain = xqc_bbr2_kDrainGain;
    bbr2->pacing_gain = xqc_bbr2_kDrainGain;
    bbr2->cwnd_gain = xqc_bbr2_kStartupCwndGain;
    xqc_bbr2_reset_congestion_signals((void*)bbr2);
}

static void xqc_bbr2_pick_probe_wait(xqc_bbr2_t *bbr2)
{
    bbr2->rounds_since_probe = random() % xqc_bbr2_bw_probe_rand_rounds;
    bbr2->probe_wait_us = xqc_bbr2_bw_probe_base_us + (random() % xqc_bbr2_bw_probe_rand_us);
}

void xqc_bbr2_set_cycle_idx(xqc_bbr2_t *bbr2, xqc_bbr2_pacing_gain_phase idx)
{
    bbr2->cycle_idx = (uint32_t)idx;
}

void xqc_bbr2_enter_probe_down(xqc_bbr2_t *bbr2, xqc_sample_t *sampler) {
...
static void xqc_bbr2_check_drain(xqc_bbr2_t *bbr2, xqc_sample_t *sampler, xqc_bbr2_context_t *ctx)
{
    if(bbr2->mode == BBR2_STARTUP && bbr2->full_bandwidth_reached)
        xqc_bbr2_enter_drain(bbr2);

    //drain to target
    if(bbr2->mode == BBR2_DRAIN && sampler->bytes_inflight <= xqc_bbr2_bdp(bbr2, xqc_bbr2_max_bw(bbr2))) {
        xqc_bbr2_enter_probe_down(bbr2, sampler);
    }
}

static uint32_t xqc_bbr2_probe_rtt_cwnd(xqc_bbr2_t *bbr2)
{
    uint32_t probe_rtt_cwnd = xqc_bbr2_inflight(bbr2, xqc_bbr2_bw(bbr2), xqc_bbr2_probe_rtt_gain);
    return xqc_max(xqc_bbr2_kMinCongestionWindow, probe_rtt_cwnd);
}

static void xqc_bbr2_exit_probe_rtt(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    xqc_bbr2_reset_lower_bounds(bbr2);
    } else {
        xqc_bbr2_enter_startup(bbr2);
...
static void xqc_bbr2_check_probe_rtt_done(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    if(!bbr2->probe_rtt_round_done_stamp || sampler->now < bbr2->probe_rtt_round_done_stamp)
        return;
...
static void xqc_bbr2_update_min_rtt(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    bool probe_rtt_expired, min_rtt_expired;
    probe_rtt_expired = sampler->now > (bbr2->probe_rtt_min_us_stamp + xqc_bbr2_probe_rtt_min_win_msec);
    if (sampler->rtt >= 0 && (sampler->rtt <= bbr2->probe_rtt_min_us || probe_rtt_expired)) {
        bbr2->probe_rtt_min_us = sampler->rtt;
        bbr2->probe_rtt_min_us_stamp = sampler->now;
    }
    min_rtt_expired = sampler->now > (bbr2->min_rtt_stamp + xqc_bbr2_kMinRttWindowSize_msec);
    ...
    if (probe_rtt_expired && !bbr2->idle_restart && bbr2->mode != BBR2_PROBE_RTT) {
    ...
    if (bbr2->mode == BBR2_PROBE_RTT) {
        ...
        xqc_log(send_ctl->ctl_conn->log, XQC_LOG_DEBUG, "|BBR PROBE_RTT|inflight:%ud|done_stamp:%ui|done:%ud|round_start:%ud|",
		...
		if (!bbr2->probe_rtt_round_done_stamp &&
			...
			bbr2->probe_rtt_round_done_stamp = sampler->now + xqc_bbr2_kProbeRttTime;
			bbr2->probe_rtt_round_done = false;
			bbr2->next_round_delivered = sampler->total_acked;
...
void xqc_bbr2_enter_probe_rtt(xqc_bbr2_t *bbr2)
{
...
void xqc_bbr2_save_cwnd(xqc_bbr2_t *bbr2)
{
...
void xqc_bbr2_restore_cwnd(xqc_bbr2_t *bbr2)
{
    bbr2->congestion_window = xqc_max(bbr2->congestion_window, bbr2->prior_cwnd);
}

static void _xqc_bbr2_set_pacing_rate_helper(xqc_bbr2_t *bbr2, float pacing_gain)
{
    uint32_t bandwidth, rate;
    bandwidth = xqc_bbr2_bw(bbr2);
    rate = bandwidth * pacing_gain * (1.0 - xqc_bbr2_pacing_rate_margin_percent);
    if(bbr2->full_bandwidth_reached || rate > bbr2->pacing_rate || bbr2->recovery_mode == BBR2_RECOVERY)
        bbr2->pacing_rate = rate;
}

static void xqc_bbr2_set_pacing_rate(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
...
static void xqc_bbr2_reset_cwnd(void *cong_ctl)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*)cong_ctl;
    xqc_bbr2_save_cwnd(bbr2);
    //cut window to the minimal
    bbr2->congestion_window = xqc_bbr2_kMinCongestionWindow;
    //enter loss state
    bbr2->recovery_mode = BBR2_LOSS;
    bbr2->last_bandwidth = 0;
    bbr2->loss_start_time = xqc_now();
    if (!xqc_bbr2_is_probing_bandwidth(bbr2) && bbr2->inflight_lo == ~0U)
        bbr2->inflight_lo = bbr2->prior_cwnd;
    //cancel & disable entering RECOVERY mode
    bbr2->recovery_start_time = 0;
}

static void xqc_bbr2_set_cwnd(xqc_bbr2_t *bbr2, xqc_sample_t *sampler, xqc_bbr2_context_t *ctx)
{
    if(sampler->acked == 0)
        goto done;
    ...
    xqc_log(send_ctl->ctl_conn->log, XQC_LOG_DEBUG,
        "|xqc_bbr2_set_cwnd|target_cwnd:%ud|extra_cwnd:%ud|current_cwnd:%ud|new_acked:%ud|",
            target_cwnd, extra_cwnd, bbr2->congestion_window, sampler->acked);
    target_cwnd += extra_cwnd;

    if(bbr2->full_bandwidth_reached) {
        bbr2->congestion_window = xqc_min(target_cwnd, bbr2->congestion_window + sampler->acked);
    }
    else if(bbr2->congestion_window < target_cwnd || bbr2->congestion_window < 2*bbr2->initial_congestion_window) {
        bbr2->congestion_window += sampler->acked;
    }
    bbr2->congestion_window = xqc_max(bbr2->congestion_window, xqc_bbr2_kMinCongestionWindow);
...
static void xqc_bbr2_on_lost(void *cong_ctl, xqc_msec_t lost_sent_time)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*)cong_ctl;
...
static void xqc_bbr2_update_pacing_gain_for_loss_recovery(void *cong_ctl)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*)cong_ctl;
    if (bbr2->mode == BBR2_STARTUP) {
        if (bbr2->recovery_mode == BBR2_RECOVERY) {
            bbr2->pacing_gain = xqc_bbr2_kStartupPacingGainOnLost;
        }
...
static void xqc_bbr2_update_recovery_mode(void *cong_ctl, xqc_sample_t *sampler)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*)cong_ctl;
        //restore cwnd
        xqc_bbr2_restore_cwnd(bbr2);
    ...
    if (sampler->po_sent_time <= bbr2->recovery_start_time && bbr2->recovery_mode == BBR2_OPEN) {
        bbr2->recovery_mode = BBR2_RECOVERY;
        //just save it. make the logic consistent with Linux Kernel
        xqc_bbr2_save_cwnd(bbr2);
    } else if (sampler->po_sent_time > bbr2->recovery_start_time && bbr2->recovery_mode == BBR2_RECOVERY) {
...
static void xqc_bbr2_take_bw_hi_sample(xqc_bbr2_t *bbr2, uint32_t bw)
{
...
static void xqc_bbr2_adapt_lower_bounds(xqc_bbr2_t *bbr2)
{
	...
	if (bbr2->loss_in_round) {
		...
		bbr2->bw_lo = xqc_max(bbr2->bw_latest, (1 - xqc_bbr2_inflight_lo_beta) * bbr2->bw_lo);
		bbr2->inflight_lo = xqc_max(bbr2->inflight_latest, (1 - xqc_bbr2_inflight_lo_beta) * bbr2->inflight_lo);
	}
}

static void xqc_bbr2_update_congestion_signals(xqc_bbr2_t *bbr2, xqc_sample_t *sampler, xqc_bbr2_context_t *ctx)
{
    uint32_t bw;
    //update bw_hi
...
bool xqc_bbr2_is_inflight_too_high(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    uint32_t loss_thresh, inflight_pkts;
    //if loss rate is too large
	if (sampler->lost_pkts > 0 && sampler->tx_in_flight) {
        inflight_pkts = sampler->tx_in_flight / XQC_kMaxDatagramSize;
...
static void xqc_bbr2_handle_queue_too_high_in_startup(xqc_bbr2_t *bbr2)
{
    bbr2->full_bandwidth_reached = 1;
    bbr2->inflight_hi = xqc_bbr2_bdp(bbr2, xqc_bbr2_max_bw(bbr2));
}

static void xqc_bbr2_check_loss_too_high_in_startup(xqc_bbr2_t *bbr2, xqc_sample_t *sampler)
{
    if(bbr2->full_bandwidth_reached) return;
...
static void xqc_bbr2_update_model(xqc_bbr2_t *bbr2, xqc_sample_t *sampler, xqc_bbr2_context_t *ctx)
{
...
static void xqc_bbr2_bound_cwnd_for_inflight_model(xqc_bbr2_t *bbr2)
{
    uint32_t cap;
    cap = ~0U;
	if (bbr2->mode == BBR2_PROBE_BW &&
	...
	cap = xqc_max(cap, xqc_bbr2_kMinCongestionWindow);
	bbr2->congestion_window = xqc_min(cap, bbr2->congestion_window);
}

static void xqc_bbr2_on_ack(void *cong_ctl, xqc_sample_t *sampler)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*)(cong_ctl);
...
static uint64_t xqc_bbr2_get_cwnd(void *cong_ctl)
{
    xqc_bbr2_t *bbr = (xqc_bbr2_t*)(cong_ctl);
    return bbr->congestion_window;
}

static uint32_t  xqc_bbr2_get_pacing_rate(void *cong_ctl)
{
    xqc_bbr2_t *bbr = (xqc_bbr2_t*)(cong_ctl);
    return bbr->pacing_rate;
}

static uint32_t xqc_bbr2_get_bandwidth(void *cong_ctl)
{
    xqc_bbr2_t *bbr = (xqc_bbr2_t*)(cong_ctl);
    return xqc_bbr2_bw(bbr);
}

static void xqc_bbr2_restart_from_idle(void *cong_ctl, uint64_t conn_delivered)
{
    xqc_bbr2_t *bbr = (xqc_bbr2_t*)(cong_ctl);
    uint32_t rate;
    uint64_t now = xqc_now();
...
static uint8_t xqc_bbr2_info_mode (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->mode;
}

static uint64_t xqc_bbr2_info_min_rtt (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->min_rtt;
}

static uint8_t xqc_bbr2_info_idle_restart (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->idle_restart;
}

static uint8_t xqc_bbr2_info_full_bw_reached (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->full_bandwidth_reached;
}

static uint8_t xqc_bbr2_info_recovery_mode (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->recovery_mode;
}

static uint64_t xqc_bbr2_info_recovery_start_time (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->recovery_start_time;
}

static uint8_t xqc_bbr2_info_packet_conservation (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->packet_conservation;
}

static uint8_t xqc_bbr2_info_round_start (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->round_start;
}

static float xqc_bbr2_info_pacing_gain (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
    return bbr2->pacing_gain;    
}

static float xqc_bbr2_info_cwnd_gain (void *cong)
{
    xqc_bbr2_t *bbr2 = (xqc_bbr2_t*) cong;
