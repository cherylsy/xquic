...
xqc_int_t xqc_ssl_init_engine_config(xqc_engine_t *engine, xqc_engine_ssl_config_t * src,
    xqc_ssl_session_ticket_key_t * session_ticket_key);

xqc_int_t xqc_ssl_init_conn_config(xqc_connection_t * conn, xqc_conn_ssl_config_t * src);
...
SSL *xqc_create_client_ssl(xqc_engine_t *engine, xqc_connection_t *conn,
    char *hostname, xqc_conn_ssl_config_t *conn_ssl_config);
...
xqc_int_t xqc_client_tls_initial(xqc_engine_t *engine, xqc_connection_t *conn,
    char *hostname, xqc_conn_ssl_config_t *conn_ssl_config,  xqc_cid_t *dcid, uint16_t no_crypto_flag);

xqc_int_t xqc_server_tls_initial(xqc_engine_t *engine, xqc_connection_t *conn, xqc_engine_ssl_config_t *sc);
