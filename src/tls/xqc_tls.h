 ...
 * trigger generating ClientHello. and for server
 * 
 ...
 * @param odcid original dcid, for calculating initial secret
 */
xqc_int_t xqc_tls_init(xqc_tls_t *tls, xqc_proto_version_t version, const xqc_cid_t *odcid);

/**
 * @brief reset initial keys, this might be called after server send Retry or client receive Retry
 */
xqc_int_t xqc_tls_reset_initial(xqc_tls_t *tls, xqc_proto_version_t version, const xqc_cid_t *odcid);
...
/**
 * @brief handle tls handshake data from peer's QUIC CRYPTO frame
 */
