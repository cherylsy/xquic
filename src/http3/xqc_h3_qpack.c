...
static size_t
xqc_table_space(size_t namelen, size_t valuelen)
{
    return XQC_QPACK_ENTRY_OVERHEAD + namelen + valuelen;
}

xqc_var_string_t *
xqc_create_var_string(uint8_t *value, size_t strlen)
{
...
void
xqc_qpack_name_value_free(xqc_qpack_name_value_t *nv)
{
    if (nv == NULL) {
        return;
    }
...
void
xqc_var_buf_free(xqc_var_buf_t *vbuf)
{
    if (vbuf) {
        xqc_free(vbuf);
    }
...
xqc_var_buf_t *
xqc_var_buf_realloc(xqc_var_buf_t *src)
{
...
size_t
xqc_http3_qpack_put_varint_len(uint64_t n, size_t prefix)
{
    size_t len = 0;
...
xqc_http3_qpack_context_dtable_get(xqc_http3_qpack_context *ctx, size_t absidx)
{
    size_t relidx;
                              ...
                              xqc_var_buf_t *dest, uint8_t *begin, uint8_t *end)
{
    ...
    ssize_t nwrite = xqc_http3_qpack_huffman_decode(&rstate->huffman_ctx, dest->data + dest->used_len, begin, len, fin);
    if (nwrite < 0) {
        return nwrite;
    }
...
ssize_t
xqc_http3_qpack_read_varint(uint64_t *pdest, uint8_t *begin, uint8_t *end, size_t prefix, int *fin)
{
    *fin = 0;
    *pdest = 0;
...
uint8_t*
xqc_http3_qpack_put_varint(uint8_t *buf, uint64_t n, size_t prefix)
{
    size_t k = (size_t)((1 << prefix) - 1);
    *buf = (uint8_t)(*buf & ~k);
...
int
xqc_http3_qpack_read_state_init(xqc_http3_qpack_read_state *rstate)
{
...
void 
xqc_h3_qpack_read_state_clear(xqc_http3_qpack_read_state *rstate)
{
...
void
xqc_http3_qpack_read_state_free(xqc_http3_qpack_read_state *rstate)
{
    if (rstate->name) {
        xqc_var_buf_free(rstate->name);
        rstate->name = NULL;
...
int
xqc_qpack_encoder_write_literal_native(xqc_http3_qpack_encoder *encoder, xqc_var_buf_t **pp_buf, uint8_t fb,
...
int
xqc_qpack_encoder_write_literal(xqc_http3_qpack_encoder *encoder, xqc_var_buf_t **pp_buf, uint8_t fb,
                                size_t prefix, char *name, size_t name_len, char *value, size_t value_len)
{
...
int
xqc_http3_qpack_encoder_write_literal(xqc_http3_qpack_encoder *encoder, xqc_var_buf_t **pp_buf, uint8_t flags,
...
int 
xqc_h3_qpack_encoder_write_header_block_prefix(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t *p_buf, size_t ricnt, size_t base)
{
    ...
    uint8_t *p = p_buf->data + p_buf->used_len;
...
ssize_t
xqc_qpack_read_varint(int *fin, xqc_http3_qpack_read_state *rstate, uint8_t *begin, uint8_t *end)
{
...
int
xqc_http3_qpack_decoder_reconstruct_ricnt(xqc_http3_qpack_decoder *decoder, size_t *dest, size_t encricnt)
{
...
xqc_qpack_decoder_emit_dynamic_indexed_name(xqc_http3_qpack_decoder *decoder, xqc_http3_qpack_stream_context *sctx, xqc_qpack_name_value_t *nv)
{
    ...
    xqc_http3_qpack_entry *entry = xqc_http3_qpack_context_dtable_get(&decoder->ctx, absidx);
...
int
xqc_qpack_decoder_emit_static_indexed_name(xqc_http3_qpack_decoder *decoder, xqc_http3_qpack_stream_context *sctx, xqc_qpack_name_value_t *nv)
{
    nv->flag = sctx->rstate.never;
                             ...
                             uint64_t max_dtable_size, uint64_t max_blocked)
{
    ...
    int ret = xqc_http3_ringdata_init(&ctx->dtable_data, max_dtable_size);
    if (ret < 0) {
...
void
xqc_http3_qpack_context_free(xqc_http3_qpack_context * ctx)
{
    xqc_http3_ringdata_free(&ctx->dtable_data);
    xqc_http3_ringbuf_free(&ctx->dtable);
}
...
void
xqc_http3_qpack_decoder_free(xqc_http3_qpack_decoder *qdec)
{
...
int
xqc_qpack_hash_table_init(xqc_qpack_hash_table_t * htable, size_t element_count)
{
...
void
xqc_qpack_hash_table_free(xqc_qpack_hash_table_t * htable)
{
    if (htable->list) {
...
int
xqc_http3_qpack_decoder_pbrel2abs(xqc_http3_qpack_decoder *decoder, xqc_http3_qpack_stream_context *sctx)
{
...
int
xqc_http3_qpack_decoder_brel2abs(xqc_http3_qpack_decoder *decoder, xqc_http3_qpack_stream_context *sctx)
{
    xqc_http3_qpack_read_state *rstate = &sctx->rstate;
    if (rstate->dynamic) {
...
int
xqc_qpack_decoder_emit_dynamic_indexed(xqc_http3_qpack_decoder *decoder, xqc_http3_qpack_stream_context *sctx, xqc_qpack_name_value_t *nv)
{
    nv->flag = sctx->rstate.never;
    uint64_t absidx = sctx->rstate.absidx;
    xqc_http3_qpack_entry *entry = xqc_http3_qpack_context_dtable_get(&decoder->ctx, absidx);
    ...
    xqc_http3_ringdata_copy_data(&decoder->ctx.dtable_data, entry->nv.value_index,
                                 entry->nv.value_len, nv->value->data, nv->value->strlen + 1);
    return 0;
}

int
xqc_qpack_decoder_emit_static_indexed(xqc_http3_qpack_decoder *decoder, xqc_http3_qpack_stream_context *sctx, xqc_qpack_name_value_t *nv)
{
    nv->flag = sctx->rstate.never;
    int absidx = sctx->rstate.absidx;
...
int
xqc_http3_qpack_decoder_emit_indexed(xqc_http3_qpack_decoder *decoder, xqc_http3_qpack_stream_context *sctx, xqc_qpack_name_value_t *nv)
{
    if (sctx->rstate.dynamic) {
...
int
xqc_qpack_decoder_block_stream_check_and_process(xqc_h3_conn_t *h3_conn, uint64_t absidx)
{
            ...
            switch (sctx->opcode) {
                case XQC_HTTP3_QPACK_RS_OPCODE_INDEXED:
                    ...
                    *pflags |= XQC_HTTP3_QPACK_DECODE_FLAG_EMIT;
                    sctx->state = XQC_HTTP3_QPACK_RS_STATE_OPCODE;
                    xqc_h3_qpack_read_state_clear(&sctx->rstate);
                    return p - src;
...
int
xqc_http3_qpack_stream_context_init(xqc_http3_qpack_stream_context *sctx, uint64_t stream_id)
{
...
int
xqc_http3_qpack_stream_context_reinit(xqc_http3_qpack_stream_context *sctx)
{
    xqc_h3_qpack_read_state_clear(&sctx->rstate);
    sctx->rstate.prefix = 8;
    sctx->state = XQC_HTTP3_QPACK_RS_STATE_RICNT;
...
void
xqc_http3_qpack_stream_context_free(xqc_http3_qpack_stream_context * sctx)
{
    if (sctx == NULL) {
        return;
    }
...
int
xqc_http3_ringdata_pop_back(xqc_http3_ringdata *rdata, xqc_http3_qpack_entry *entry)
{
    int ret = xqc_http3_ringdata_out_queue(rdata, entry->nv.name_index, entry->nv.name_len);
    if (ret < 0) {
...
int
xqc_http3_qpack_dtable_pop(xqc_http3_qpack_context *ctx)
{
    int i = xqc_http3_ringbuf_len(&ctx->dtable);
...
int
xqc_http3_qpack_encoder_reduce_dtable_size(xqc_http3_qpack_encoder *encoder, size_t cap)
{
...
int
xqc_http3_qpack_encoder_expand_dtable_size(xqc_http3_qpack_context *ctx, size_t cap)
{
...
int
xqc_qpack_write_number(xqc_var_buf_t **pp_buf, uint8_t fb, uint64_t num, size_t prefix)
{
...
int
xqc_http3_qpack_decoder_rel2abs(xqc_http3_qpack_decoder * decoder, xqc_http3_qpack_read_state * rstate)
{
    if (rstate->dynamic) {
...
int
xqc_http3_qpack_entry_init(xqc_http3_qpack_entry *ent, size_t name_index, size_t name_len,
                           size_t value_index, size_t value_len, size_t sum, size_t absidx)
{
...
int
xqc_http3_qpack_hash_find(xqc_qpack_hash_table_t *table, xqc_http3_ringdata *rdata, char *name, size_t name_len,
                          char *value, size_t value_len, xqc_qpack_find_result *result)
{
    ...
    xqc_list_for_each_safe(pos, next, head) {
        ...
        if ((name_len == entry->nv.name_len
            && 0 == xqc_http3_ringdata_cmp(rdata, entry->nv.name_index, name, entry->nv.name_len)))
        {
            if (result->name_entry == NULL) {
                result->name_entry = entry;
            }

            if ((value_len == entry->nv.value_len)
                && (0 == xqc_http3_ringdata_cmp(rdata, entry->nv.value_index, value, entry->nv.value_len)))
            {
                result->entry = entry;
                break;
            }
...
int
xqc_http3_qpack_stable_find(xqc_qpack_static_find_result * s_result, int token, uint8_t * name, size_t name_len, 
...
int
xqc_http3_qpack_hash_insert(xqc_qpack_hash_table_t * table, char *name, size_t name_len, xqc_list_head_t * head_list)
{
...
int
xqc_http3_qpack_hash_del(xqc_list_head_t * head_list)
{
...
int
xqc_http3_qpack_context_dtable_add(xqc_http3_qpack_context *ctx, uint8_t *name, size_t name_len, 
                                   uint8_t *value, size_t value_len, xqc_qpack_hash_table_t *dtable_hash)
{
    size_t space;
    int i,rv, ret = 0 ;
...
int
xqc_http3_qpack_decoder_dtable_duplicate_add(xqc_http3_qpack_decoder * decoder)
{
    int rv = 0;
    xqc_http3_qpack_entry *entry = xqc_http3_qpack_context_dtable_get(&decoder->ctx, decoder->rstate.absidx);
...
int
xqc_http3_qpack_decoder_dtable_dynamic_add(xqc_http3_qpack_decoder * decoder)
{
    xqc_http3_qpack_entry *entry;
...
int
xqc_http3_qpack_decoder_dtable_static_add(xqc_http3_qpack_decoder * decoder)
{
...
int
xqc_http3_qpack_decoder_dtable_indexed_add(xqc_http3_qpack_decoder * decoder)
{
...
int
xqc_http3_qpack_decoder_dtable_literal_add(xqc_http3_qpack_decoder * decoder)
{
...
ssize_t
xqc_http3_qpack_decoder_read_encoder(xqc_h3_conn_t *h3_conn, uint8_t * src, size_t srclen, int *insert_count)
{
    uint8_t * p = src, * end = src + srclen;
    int rv = 0;
    ssize_t nread;
        ...
        switch(decoder->state){
            ...
            case XQC_HTTP3_QPACK_ES_STATE_READ_INDEX:
                ...
                if (decoder->opcode == XQC_HTTP3_QPACK_ES_OPCODE_SET_DTABLE_CAP) {
                    ...
                    xqc_h3_qpack_read_state_clear(&decoder->rstate);
                    break;
                }
...
int 
xqc_h3_qpack_encoder_add_insert_count(xqc_http3_qpack_encoder * encoder, size_t n)
{
...
int 
xqc_h3_qpack_check_and_refresh_insert_count(xqc_http3_qpack_encoder * encoder, size_t max_rcnt)
{
...
int 
xqc_h3_qpack_encoder_insert_unack_header(xqc_h3_stream_t * qenc_stream, 
    xqc_h3_stream_t *h3_stream, xqc_http3_qpack_encoder * encoder,
    uint64_t min_rcnt, uint64_t max_rcnt)
{
    ...
    xqc_stream_refcnt_add(h3_stream->stream);
...
int 
xqc_h3_qpack_encoder_ack_header(xqc_h3_conn_t * h3_conn, uint64_t stream_id)
{
    xqc_connection_t * conn = h3_conn->conn;
    xqc_stream_t * stream = xqc_find_stream_by_id(stream_id, conn->streams_hash);
    xqc_h3_stream_t * h3_stream = NULL;

    if (stream == NULL) {
        xqc_log(conn->log, XQC_LOG_ERROR, 
                "|xqc_h3_qpack_encoder_ack_header: find NULL stream, stream_id:%lu|", stream_id);
        /* log & don't return err */
        return XQC_OK;
    }

    h3_stream = stream->user_data;
    ...
    xqc_stream_refcnt_del(h3_stream->stream);
...
void 
xqc_h3_qpack_free_unack_header_block(xqc_qpack_unack_header_block * header_block)
{
...
int 
xqc_h3_qpack_clear_block_stream_list(xqc_h3_conn_t *h3_conn, uint64_t stream_id)
{
    xqc_list_head_t * head = &(h3_conn->block_stream_head);
...
int 
xqc_h3_stream_clear_unack_and_block_stream_list(xqc_h3_stream_t * h3_stream)
{
    xqc_list_head_t * head = &(h3_stream->unack_block_list);
    ...
    xqc_list_for_each_safe(pos, next, head) {
        ...
        xqc_h3_qpack_free_unack_header_block(header_block);
        xqc_stream_refcnt_del(h3_stream->stream);
    }
    ...
    xqc_h3_qpack_clear_block_stream_list(h3_conn, stream_id);
...
int 
xqc_h3_qpack_encoder_cancel_stream(xqc_h3_conn_t *h3_conn , uint64_t stream_id)
{
    xqc_connection_t * conn = h3_conn->conn;
    xqc_stream_t * stream = xqc_find_stream_by_id(stream_id, conn->streams_hash);
...
ssize_t 
xqc_h3_qpack_encoder_read_decoder(xqc_h3_conn_t * h3_conn, uint8_t * src, size_t srclen)
{
        ...
        switch(encoder->state){
            ...
            case XQC_HTTP3_QPACK_DS_STATE_READ_NUMBER:
                ...
                case XQC_HTTP3_QPACK_DS_OPCODE_ICNT_INCREMENT:
                    rv = xqc_h3_qpack_encoder_add_insert_count(encoder, encoder->rstate.left);
                    if (rv != XQC_OK) {
                        goto fail;
                    }
                    break;
                case XQC_HTTP3_QPACK_DS_OPCODE_HEADER_ACK:
                    rv = xqc_h3_qpack_encoder_ack_header(h3_conn, (int64_t)encoder->rstate.left);
                    if (rv != XQC_OK) {
                        goto fail;
                    }
                    break;
                case XQC_HTTP3_QPACK_DS_OPCODE_STREAM_CANCEL:
                    rv = xqc_h3_qpack_encoder_cancel_stream(h3_conn, (int64_t)encoder->rstate.left);
                    if (rv != XQC_OK) {
                        goto fail;
                    }
                    break;
                ...
                xqc_h3_qpack_read_state_clear(&encoder->rstate);
                break;
...
int 
xqc_h3_qpack_context_check_draining(xqc_http3_qpack_context * ctx, 
    xqc_http3_qpack_entry * entry)
{
...
int 
xqc_h3_qpack_encoder_dtable_prepare_space(xqc_http3_qpack_encoder * encoder, 
    size_t need)
{
...
int 
xqc_h3_qpack_encoder_can_index(xqc_http3_qpack_encoder *encoder, 
    size_t need)
{
    size_t avail = 0;
...
int 
xqc_h3_qpack_encoder_can_index_nv(xqc_http3_qpack_encoder *encoder, 
    size_t name_len, size_t value_len)
{
    size_t need = xqc_table_space(name_len, value_len);
    xqc_h3_qpack_encoder_dtable_prepare_space(encoder, need);
    return xqc_h3_qpack_encoder_can_index(encoder, need);
}


int 
xqc_h3_qpack_encoder_write_static_indexed(xqc_http3_qpack_encoder * encoder,
    xqc_var_buf_t **pp_buf, uint64_t absidx)
{
    int ret = 0;
...
xqc_h3_qpack_encoder_decide_indexing_mode(xqc_http3_qpack_encoder * encoder, 
    int * token, char * name, size_t name_len,
    char *value, size_t value_len, uint8_t flags) 
{
...
int 
xqc_h3_qpack_encoder_write_dynamic_indexed(xqc_http3_qpack_encoder * encoder, 
    xqc_var_buf_t **pp_buf, xqc_http3_qpack_entry *entry, size_t base)
{
...
int 
xqc_h3_qpack_encoder_write_indexed_name(xqc_http3_qpack_encoder * encoder, 
    xqc_var_buf_t **pp_buf, uint8_t fb,
...
int 
xqc_h3_qpack_encoder_write_static_insert(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t ** p_enc_buf, size_t nameidx, uint8_t * value, size_t value_len )
{
    return xqc_h3_qpack_encoder_write_indexed_name(encoder, p_enc_buf, 0xc0, nameidx, 6, value, value_len);
}

int 
xqc_h3_qpack_encoder_write_static_indexed_name(xqc_http3_qpack_encoder * encoder, 
    xqc_var_buf_t **pp_buf, uint64_t absidx, char * value, size_t value_len, uint8_t flags)
{
    uint8_t fb = (uint8_t)(0x50 | ((flags & XQC_HTTP3_NV_FLAG_NEVER_INDEX) ? 0x20 : 0));
    return xqc_h3_qpack_encoder_write_indexed_name(encoder, pp_buf, fb, absidx, 4, value, value_len);
}


int 
xqc_h3_qpack_encoder_write_duplicate_insert(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t ** p_enc_buf, xqc_http3_qpack_entry *entry)
{
...
int xqc_h3_qpack_encoder_dtable_duplicate_add(xqc_http3_qpack_encoder * encoder, 
    xqc_http3_qpack_entry *entry)
{
...
int xqc_h3_qpack_encoder_write_dynamic_insert(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t ** p_enc_buf, size_t nameidx, uint8_t * value, size_t value_len)
{
    return xqc_h3_qpack_encoder_write_indexed_name(encoder, p_enc_buf, 0x80,  encoder->ctx.next_absidx - nameidx - 1, 6, value, value_len);
}

int 
xqc_h3_qpack_encoder_write_dynamic_indexed_name(xqc_http3_qpack_encoder * encoder, xqc_var_buf_t **pp_buf,
    uint64_t absidx, size_t base, char * value, size_t value_len, uint8_t flags)
{
    ...
    if (absidx < base) {
        ...
        return xqc_h3_qpack_encoder_write_indexed_name(encoder, pp_buf, fb, base - absidx - 1, 4, value, value_len);

    } else {
        fb = (flags & XQC_HTTP3_NV_FLAG_NEVER_INDEX) ? 0x08 : 0;
        return xqc_h3_qpack_encoder_write_indexed_name(encoder, pp_buf, fb, absidx - base, 3, value, value_len);
    }
}

int 
xqc_h3_qpack_encoder_write_literal_insert(xqc_http3_qpack_encoder *encoder,  
    xqc_var_buf_t **p_enc_buf, char *name, size_t name_len, 
...
int 
xqc_h3_qpack_encoder_dtable_duplicate(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t ** p_enc_buf, xqc_http3_qpack_entry *entry)
{
    int can_insert = xqc_h3_qpack_encoder_can_index_nv(encoder, entry->nv.name_len, entry->nv.value_len);

    if (can_insert) {
        xqc_h3_qpack_encoder_write_duplicate_insert(encoder, p_enc_buf, entry); //通知对端更新需要放在插入之前，因为插入会改变absidx,而通知对端计算相对index是根据插入前的absidx计算
        xqc_h3_qpack_encoder_dtable_duplicate_add(encoder, entry);
    }
...
int 
xqc_h3_qpack_encoder_dtable_static_write(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t ** p_enc_buf, size_t nameidx, char * name, size_t name_len, 
    char * value, size_t value_len)
{
    int can_insert = xqc_h3_qpack_encoder_can_index_nv(encoder, name_len, value_len);

    if (can_insert) {
        xqc_h3_qpack_encoder_write_static_insert(encoder, p_enc_buf, nameidx, value, value_len);
...
int 
xqc_h3_qpack_encoder_dtable_dynamic_write(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t ** p_enc_buf, size_t nameidx, char *name, size_t name_len, 
    char * value, size_t value_len)
{
    int can_insert = xqc_h3_qpack_encoder_can_index_nv(encoder, name_len, value_len);
    ...
    int ret = 0;
    if (can_insert) {
        ret = xqc_h3_qpack_encoder_write_dynamic_insert(encoder, p_enc_buf, nameidx, value, value_len);
        if(ret < 0){
            return ret;
        }
...
int 
xqc_h3_qpack_encoder_dtable_literal_write(xqc_http3_qpack_encoder *encoder, 
    xqc_var_buf_t ** p_enc_buf, char *name, size_t name_len, 
    char * value, size_t value_len)
{
    int can_insert = xqc_h3_qpack_encoder_can_index_nv(encoder, name_len, value_len);
    ...
    if (can_insert) {
        xqc_h3_qpack_encoder_write_literal_insert(encoder, p_enc_buf, name, name_len, value, value_len);
    ...
    xqc_http_header_t * header, size_t base)
{
    ...
    xqc_http3_qpack_indexing_mode indexing_mode = xqc_h3_qpack_encoder_decide_indexing_mode(encoder, &token, 
                                                     name, name_len, value, value_len, flags);
...
int 
xqc_h3_headers_save_nv(xqc_http_headers_t * headers, 
    xqc_qpack_name_value_t * nv)
{
    if (NULL == nv->name || NULL == nv->value) {
        return -1;
    }
...
int 
xqc_h3_qpack_decoder_write_header_ack(xqc_h3_stream_t * qdec_stream, 
    uint64_t stream_id)
{
...
int 
xqc_h3_qpack_decoder_write_insert_count_increment(xqc_h3_stream_t * qdec_stream, 
    size_t insert_count)
{
...
int 
xqc_h3_handle_header_data_streaming(xqc_h3_conn_t *h3_conn,
    xqc_h3_stream_t * h3_stream, char * data, size_t len, uint8_t fin_flag)
{
    int nread = 0;
    ...
    while (start < end) {
        ...
        if (flags & XQC_HTTP3_QPACK_DECODE_FLAG_EMIT) {
            //save nv
            if (xqc_h3_headers_save_nv(headers, &nv) < 0) {
    ...
    if (start == end) {
        ...
        if (fin_flag & XQC_HTTP3_FRAME_FIN) {
            ...
            if (sctx->ricnt > 0) {
                xqc_h3_qpack_decoder_write_header_ack(h3_conn->qdec_stream, h3_stream->stream->stream_id);
