...
xqc_bool_t
xqc_encoder_check_insert(xqc_encoder_t *enc, xqc_hdr_enc_rule_t* info, uint64_t stream_id)
{
    ...
    xqc_bool_t blocked = xqc_encoder_check_block_stream_limit(enc, stream_id);  // TODO: 优化，外部有，不需要重复
    ...
    {
        /* impossible here */
        xqc_log(enc->log, XQC_LOG_WARN, "|dtable shall not return an entry with index >= krc|");
    ...
    uint64_t stream_id)
{
    ...
    /* check if insertion is possible */
    if (xqc_encoder_check_insert(enc, info, stream_id) == XQC_FALSE) {
        return;
    }
...
xqc_encoder_insert(xqc_encoder_t *enc, xqc_hdr_enc_rule_t *info, xqc_var_buf_t *ins)
{
    ...
    if (ret != XQC_OK) {
        ...
        info->insertion = XQC_INSERT_NONE;
        xqc_log(enc->log, XQC_LOG_WARN, "|insertion failed|ret:%d|", ret);
...
xqc_encoder_try_duplicate(xqc_encoder_t *enc, xqc_hdr_enc_rule_t *info, xqc_var_buf_t *ins)
{
    ...
    if (draining == XQC_TRUE) {
        ...
        if (ret != XQC_OK) {
            xqc_log(enc->log, XQC_LOG_DEBUG, "|duplicate entry error|ret:%d|", ret);
            return ret;
        }
        ...
        /* change info->index until duplicate success, if it fails, will do as lookup result */
        info->index = dup_idx;
    }
    ...
    xqc_var_buf_t *ins, uint64_t stream_id)
{
    ...
    xqc_dtable_set_lookup_limited(enc->dtable, limited);
    ...
    for (size_t i = 0; i < hdrs->count; i++) {
        ...
        if (info->insertion != XQC_INSERT_NONE) {
                ...
                && info->ref_table == XQC_DTABLE_FLAG)
            {
                ...
                xqc_encoder_try_duplicate(enc, info, ins);
            }
        }
            ...
            && info->index < fs->min_ref_idx)
        {
            ...
            xqc_dtable_set_min_ref(enc->dtable, info->index);
        }
...
xqc_encoder_section_ack(xqc_encoder_t *enc, uint64_t stream_id)
{
    ...
    enc->min_unack_index = XQC_INVALID_INDEX;   /* set it to max first TODO: 如果section->min_rep_index > enc->min_unack_index，则不需要更新 min_rep_index */
    ...
    if (found == XQC_FALSE) {
        ...
        // TODO: might optimize whether able to unblock streams
        ...
        xqc_dtable_set_min_ref(enc->dtable, enc->min_unack_index);
        xqc_dtable_set_known_rcvd_cnt(enc->dtable, enc->krc);
        xqc_dtable_set_lookup_limited(enc->dtable, enc->blocked_stream_count == enc->max_blocked_stream);
        return XQC_OK;
    }
}
...
xqc_encoder_cancel_stream(xqc_encoder_t *enc, uint64_t stream_id)
{
    ...
    xqc_list_for_each_safe(pos, next, &enc->unack_list) {
        xqc_encoder_unack_section_t *section = xqc_list_entry(pos, xqc_encoder_unack_section_t, head);
    ...
    xqc_list_for_each_safe(pos, next, &enc->blocked_list) {
        xqc_encoder_blocked_stream_t *stream = xqc_list_entry(pos, xqc_encoder_blocked_stream_t, head);
...
xqc_encoder_increase_known_rcvd_count(xqc_encoder_t *enc, uint64_t increment)
{
    ...
    enc->krc += increment;
    xqc_int_t ret = xqc_dtable_set_known_rcvd_cnt(enc->dtable, enc->krc);
    if (ret != XQC_OK) {
        xqc_log(enc->log, XQC_LOG_ERROR, "|insert count increment illegal|krc:%ui|increment:%ui|"
