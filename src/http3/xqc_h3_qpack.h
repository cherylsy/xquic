...
#include "src/http3/xqc_h3_ringbuf.h"
...
#define XQC_QPACK_ENTRY_OVERHEAD                32
...
#define XQC_HTTP3_QPACK_MAX_VALUELEN (32*1024) //16K enough? //修改成配置项
...
ssize_t xqc_h3_qpack_encoder_read_decoder(xqc_h3_conn_t * h3_conn, uint8_t * src, size_t srclen);

int xqc_h3_qpack_encoder_init(xqc_http3_qpack_encoder *qenc, uint64_t max_table_capacity, uint64_t max_dtable_size,
        uint64_t max_blocked, size_t hash_table_size, xqc_h3_conn_t * h3_conn);
int xqc_h3_qpack_decoder_init(xqc_http3_qpack_decoder *qdec, uint64_t max_table_capacity, uint64_t max_dtable_size, uint64_t max_blocked, xqc_h3_conn_t * h3_conn);

int xqc_qpack_decoder_block_stream_check_and_process(xqc_h3_conn_t *h3_conn, uint64_t absidx);
int xqc_h3_qpack_decoder_write_insert_count_increment(xqc_h3_stream_t * qdec_stream, size_t insert_count);


int xqc_h3_handle_header_data_streaming(xqc_h3_conn_t *h3_conn,  xqc_h3_stream_t * h3_stream, char * data, size_t len, uint8_t fin_flag);
...
int xqc_h3_stream_clear_unack_and_block_stream_list(xqc_h3_stream_t * h3_stream);
#endif
