...
recv_handler(int fd, short what, void *arg)
{
    ...
    for (; idx < send_buff_idx; idx++) {
        ...
        for (int i = 0; i < send_buff_len[idx]; i++) {
            printf("%x ", send_buff[idx][i]);
        }
        printf("\n");

        if (xqc_engine_packet_process(ctx->engine, ctx->conn, send_buff[idx], send_buff_len[idx],
...
int main(int argc, char *argv[]) {
    ...
    ctx.conn = xqc_connect(ctx.engine, NULL);
    if (!ctx.conn) {
